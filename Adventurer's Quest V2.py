...
def stage5():
    global char_x, char_y, boss_health, player_health, print_once
...
fish_list = [
    {"x": -150, "y": -100, "jump_height": 40, "jump_speed": 1, "vertical_velocity": 1, "is_jumping": True,
     "health": 100},
...
# List to track fish projectiles
fish_projectiles = []
fish_projectiles.append({
    "x": -150,
    "y": -100,
    "dx": 0,  # Move only vertically for now
    "dy": -3,  # Speed
    "radius": 3
})


def launch_fish_attack(fish_x, fish_y, player_x, player_y):
    """Launch a projectile from the fish towards the player."""
    dx = player_x - fish_x
    dy = player_y - fish_y
    ...
    fish_projectiles.append({
        "x": fish_x,
        "y": fish_y,
        "dx": dx * 3,  # Speed factor
        "dy": dy * 3,  # Speed factor
        "radius": 3  # Projectile size
    })
    print(f"Fish attack launched at ({fish_x}, {fish_y}) towards ({player_x}, {player_y})")


def move_fish_projectiles():
    """Update the position of all fish projectiles."""
    for projectile in fish_projectiles[:]:
        projectile["x"] += projectile["dx"]
        projectile["y"] += projectile["dy"]
        # Remove projectiles that go off-screen
        if abs(projectile["x"]) > W_Width / 2 or abs(projectile["y"]) > W_Height / 2:
            fish_projectiles.remove(projectile)


def draw_fish_projectiles():
    """Draw the fish projectiles."""
    for projectile in fish_projectiles:
        draw_circle(projectile["x"], projectile["y"], projectile["radius"], (0.5, 0, 1))  # Purple color

damage_reduction = 0.5  # Reduces damage by 50%

def check_collision_with_fish_projectiles(player_x, player_y):
    """Check if the player is hit by any fish projectile."""
    global player_health, damage_reduction
    for projectile in fish_projectiles[:]:
        if check_collision_circle(player_x, player_y, 5, projectile["x"], projectile["y"], projectile["radius"]):
            damage = 100  # Base damage
            if stage == 3:  # Reduce damage in Stage 3
                damage *= (1 - damage_reduction)
            player_health -= damage
            print(f"Player hit by fish attack! Health: {player_health}")
            fish_projectiles.remove(projectile)
        ...
        if fish["health"] > 0:  # Only check collisions with alive fish
            ...
            if (
                ...
                if stage == 3:  # Reduce damage in Stage 3
                    damage *= (1 - damage_reduction)
                player_health -= damage
                print(f"Player hit by fish! Health: {player_health}")
                return True
    return False
...
def update_and_draw_fish(fish_list, player_x, player_y):
    for fish in fish_list:
        update_fish(fish)
        draw_fish(fish)
        if random.random() < 0.02:  # Random chance to attack
            launch_fish_attack(fish["x"], fish["y"], player_x, player_y)
...
dodged_attacks = 0
special_attack_enabled = False
special_attack_damage = 2000  # High damage for special attack

def check_collision_with_poison(player_x, player_y):
    global player_health, dodged_attacks, special_attack_enabled

    for poison in poisons[:]:
        if check_collision1(player_x, player_y, poison["x"], poison["y"], 2):  # Adjust radius as needed
            # Player hit by poison
            player_health -= 100
            dodged_attacks = 0  # Reset dodged counter
            print(f"Player hit by poison! Health: {player_health}")
            poisons.remove(poison)
            return
    # If no collision occurs, increment dodged attacks
    dodged_attacks += 1

    if dodged_attacks >= 3:
        special_attack_enabled = True
        return
...
def update():
    ...
    if stage == 3:
        # Aura logic for Stage 3
        draw_circle(char_x + 4, char_y - 5, 12, (0.0, 1.0, 0.0))  # Aura rendering
        move_fish_projectiles()
        check_collision_with_fish_projectiles(char_x, char_y)
        if check_collision_with_fish(char_x, char_y, fish_list):
            if player_health > 0:
                print(f"Player Health: {player_health}")
            else:
                stage = 5  # Transition to Game Over screen
                print("Game Over! Player Health Depleted!")
    ...
    if stage == 4:
        move_projectiles()  # Move projectiles fired by the boss
        draw_projectiles()  # Render the projectiles on screen

        # Check if player's attacks hit the boss
        for attack in attacks[:]:
            if check_collision_circle(attack["x"], attack["y"], attack["radius"], 150, 0, 50):  # Boss hit
                boss_health -= attack["damage"]  # Apply the attack's damage
                attacks.remove(attack)  # Remove the attack after it hits
        ...
        for proj in projectiles[:]:
            if check_collision1(char_x, char_y, proj[0], proj[1], 5):
                 ...
                 projectiles.remove(proj)
                 update_health()
        # Check for boss defeat or player death
        if boss_health <= 0:
            print("Boss defeated!")
            stage = 5  # Transition to the victory screen
        elif player_health <= 0:
            print("Game Over! Player defeated!")
            stage = 5  # Transition to the game over screen
...
def keyboard(key, x, y):
    global char_velocity_x, on_ground, char_velocity_y, attacks, facing_left, stage, game_paused, special_attack_enabled
    if game_paused == True:
        return
    if key == b'a':  # Move left
        ...
        attacks.append({
            ...
            "direction": direction,
            "speed": 8,
            "damage": 100  # Regular attack damage
        })
        launch_sparks(char_x, char_y, count=10)

    elif key == b'g' and special_attack_enabled:  # Special attack
        direction = -1 if facing_left else 1
        attacks.append({
            "x": char_x + (10 * direction),
            "y": char_y,
            "radius": 6,  # Larger radius for special attack
            "direction": direction,
            "speed": 10,
            "damage": special_attack_damage, # High damage
            "color": (0, 0, 255),  # Change color to red (you can use RGB tuples for colors)

        })
        special_attack_enabled = False  # Disable special attack after use
        print("Special Attack Launched!")
...
print_once = 1
def display():
    global stage, coins, fish_list, fish_projectiles, print_once
    ...
    if stage == 1:
        ...
        draw_hollow_triangle()
        draw_fish_projectiles()
        # Draw the player's aura
        draw_circle(char_x + 4, char_y - 5, 20, (1.0, 0.0, 0.0))  # Green aura
        draw_circle(char_x + 4, char_y - 5, 17, (0.0, 0.0, 1.0))  # Green aura
        ...
        move_projectiles()
        draw_projectiles()
        if dodged_attacks >= 3 and print_once<2:
            print_once += 1
            print("Special Attack Enabled!")
    elif stage == 5:
        glClearColor(0.0, 0.0, 0.0, 1)
        stage5()
...
def mouseListener(button, state, x, y):
    global game_paused, W_Height, is_paused
